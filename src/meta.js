 ...
 *
 * @function
 * @name ILifeCircleHook#onContainerInit
 *
 * @param {IoC} ioc ioc 实例
 * @param {IoCConfig} ioc 容器配置
 *
 ...
 *
 * @function
 * @name ILifeCircleHook#onAddComponent
 *
 * @param {IoC} ioc ioc 实例
 ...
 *
 * @function
 * @name ILifeCircleHook#onGetComponent
 *
 * @param {IoC} ioc ioc 实例
 ...
 *
 * @function
 * @name ILifeCircleHook#beforeCreateInstance
 *
 * @param {IoC} ioc ioc 实例
 ...
 * 返回一个值为实例的 promise 给 ioc 使用，若返回值不为 promise，则不会覆盖现有实例。
 *
 * @function
 * @name ILifeCircleHook#beforeCreateInstance
 *
 ...
 * 批量组件配置, 其中每个key 为组件 id，值为构建配置对象，配置选项见 @link ComponentConfig
 *
 * @param {Plugin[]} [config.plugins] ioc 插件
 */
 ...
 * @property {Array} $list 声明数组形式的依赖，获取组件时，会创建一个数组，数组元素根据其对应$list中所声明的配置进行创建
 * @property {Object} $map 声明对象（映射表）形式的依赖，获取组件时，会创建一个对象，对象的属性根据其对应$map中所声明的配置进行创建
 */
